...
description: >
  ...
  and implement each with one cheap grok-4.6 worker (no inner review swarm).
  Epics/parents expand to their lowest-numbered eligible child (never implement
  the epic shell). Count mode: /solve or /solve 1 solves one issue (default);
  /solve N solves up to N issues; /solve all drains every eligible unblocked
  leaf until none remain (no soft cap such as 5; mandatory Linear re-check
  before exit). Multi-issue runs (/solve all, /solve N with N≥2, any fast run)
  first build batch guidance that inventories issues, detects platform/
  supersession conflicts (e.g. older ClickHouse/Convex tickets vs newer Neon
  migration), orders migrations before features that would be rewritten, and
  skips or re-scopes obsolete open tickets. Fast mode solves remaining issues
  in parallel after that guidance (worktree workers, orchestrator merges to
  local dev, mandatory cleanup). Always refresh from latest main, keep local
  dev updated, complete work on a short-lived branch, verify, commit, and
  merge into local dev only—do not push or open a PR unless the user asks.
  Remainder text that names a Linear milestone, label, or area (e.g. /solve
  design related issues) drains that cluster one leaf at a time until none
  remain in scope. Auto-resolve Linear team and project from repo context. Use
  when the user runs /solve, /solve N, /solve all, /solve all fast, /solve
  <milestone or area>, says "solve the next issue", "pick up the next ticket",
  "solve design related issues", or wants open issue(s) completed onto local dev.
argument-hint: "[N|all] [fast] [--concurrency N] [--effort N] [ISSUE-ID] [milestone|area…] [extra constraints…]"
---

# /solve — Next Unblocked Linear Issue(s) → Local `dev`

Select unsolved, **unblocked** Linear issue(s) for the active repo’s Linear project, implement each with **one `solve-implementer`** (low reasoning; optional one bug-only reviewer on heavy/critical — [`../docs/intensity.md`](../docs/intensity.md)), verify, commit, and **merge into the long-lived local `dev` branch** (lowercase). Default delivery stops there. `/prb` is the deep review.
...
When `SOLVE_COUNT_MODE = all` (sequential or fast; project-wide **or** scoped):
...
2. **`all` ≠ `5`.** Inner-review is auto-dialed per issue from [`../docs/intensity.md`](../docs/intensity.md) (or `--effort N`, capped at 1). Default **fast concurrency** is **2**. Neither is a solve count. Only a bare integer token sets count mode to `N`.
3. **Continue after every success** until Phase 2 (or F6 refill) finds **zero** eligible unblocked implementable leaves **in the active set** (`SCOPE` if set, else the project).
4. **Mandatory drain gate before Phase 9:** re-query Linear with the eligibility **Linear inventory fetch** (team + project + state per actionable status, page each state). Re-apply **scope filter**, eligibility (2B), blocked (2D), epic expand (2E), and guidance skip set. If any implementable leaf remains in the active set → **do not** write the final batch summary; resume the batch loop / ready-queue immediately. A scoped run may stop while the rest of the project still has eligible leaves.
...
- **Batch size** (how many issues this run solves):
  ...
  - **Integer `N`**: `/solve N` solves up to **N** eligible issues (sequential: one at a time; fast: up to N with parallelism). Inside a scope, `N` is a cap on that cluster.
  - **`all`**: **drain** — keep going until **no** eligible unblocked implementable leaves remain **in the active set** (whole project, or `SCOPE` if set). See [Drain contract](#drain-contract-solve-all--non-negotiable). No soft cap. Mandatory Linear re-check before exit.
  - **Scope** (milestone / label / area remainder): resolve per [`references/eligibility.md`](references/eligibility.md) **Scope filter**. A resolved `SCOPE` with no bare `N` sets count mode to **`all` for that cluster only** — sequential one leaf at a time until the scope is drained. Do not pick outside `SCOPE`.
...
- **Epics / parents**: never implement an epic (or any issue that has child issues) as the work item. Expand to the lowest-numbered eligible child ([`references/eligibility.md`](references/eligibility.md)). Recurse if that child is itself a parent. If **all children are completed** (Done/Canceled/Duplicate/Completed), mark the epic **Done** (rollup) and continue — does not consume a solve-count slot.
...
- **Implementation**: **one** `solve-implementer` subagent applying the Linear contract + [`references/custom-implement-instructions.md`](references/custom-implement-instructions.md). Do **not** run bundled `/implement` until-zero-nits. Optional **one** `solve-reviewer` on heavy/critical (bugs only; nits do not block). The solve orchestrator **does not** write or patch application source itself during Phase 5 (sequential) or during worker implement loops (fast — workers host construction; orchestrator still does not edit app source except merge conflict resolution on `dev`).
...
- **Git delivery default** (per issue):
  ...
  4. Run cheap construction + verify + commit on that branch
...
| `all` | **Drain** every eligible unblocked implementable leaf in the active set (project, or `SCOPE`). No soft cap. Mandatory re-query Linear before Phase 9; only real stop conditions apply (see [Drain contract](#drain-contract-solve-all--non-negotiable)). |
...
| `--concurrency M` | Fast only. Max simultaneous workers (1–8). Default: `min(N, 8)` in integer mode; **2** in `all fast` mode. Ignored (warn once) if not in fast mode. |
...
| `TEAM-123` | Prefer this issue for the **first** slot if unblocked (still validate). If `SCOPE` is set, the id must be in scope. Sequential: later slots re-select inside the active set. Fast: include in inventory when eligible. |
| other text | **Scope first**, then extra implementer constraints. Resolve leftover tokens as a Linear **milestone**, then **label**, then title/label/id **area** ([`references/eligibility.md`](references/eligibility.md) Scope filter). A hit drains that cluster (`all` unless `N` was given). If it is clearly extra constraints and matches no cluster, keep default count **1**. |
...
4. Extract count mode from the remaining tokens:
   ...
   - If neither is present → count mode **unset** (default **`1`** unless Phase 1.5 sets `SCOPE`).
5. Treat a `TEAM-\d+` token as preferred issue id (first issue / inventory pin).
6. Remainder = `REST` (scope query and/or extra constraints). Resolve `SCOPE` in **Phase 1.5** after team/project exist. Then:
...
**Disambiguation:** `--effort 3` never sets the solve count to 3. `--concurrency 3` never sets the solve count to 3. `/solve 3` means three issues (inner-review per stamp). `/solve 3 fast` means up to three issues in fast mode with concurrency up to 3. `/solve all fast --concurrency 5` drains the active set with **width** 5 (not a cap of 5 issues). `/solve all` / `/solve all fast` means **unlimited until drained** — never treat `--effort` or concurrency **2** as “solve only two tickets.” `/solve design related issues` is a **scoped drain**, not extra constraints and not a project-wide `all`.
...
`/solve`, `/solve N`, `/solve all`, `/solve all fast`, `/solve N fast`, `/solve design related issues`, `/solve the Design milestone`, `solve the next issue`, `solve the next N issues`, `solve all open issues`, `solve all fast`, `solve <milestone or area> related issues`, `parallel solve`, `pick up the next ticket`, `work the next unblocked Linear issue`, `next lowest Linear issue`, `complete the next open ticket onto dev`
...
5. Parse invocation → set **`SOLVE_COUNT_MODE`**, **`FAST_MODE`**, **`CONCURRENCY`**, **`REST`**:
   - `all` if user passed `all`
   - positive integer `N` if user passed bare `N`
   - else count **unset** until Phase 1.5
   - `FAST_MODE` if `fast` / `--fast`
   - `CONCURRENCY` from `--concurrency` or defaults (see [Concurrency defaults](#concurrency-defaults-fast-only); `all fast` default is **2**)
   - Also parse `--effort`, preferred `TEAM-123`, and `REST` (see [Args](#args)).
6. Initialize batch trackers:
   - `SOLVED = []` (list of successfully closed issues this run)
   - `FAILED = []` / `SKIPPED = []` / `SKIPPED_AT_END = []` as needed
   - `ATTEMPTED = 0`
   - `GUIDANCE_REQUIRED` = true when `SOLVE_COUNT_MODE` is `all` (including scoped drain), or integer `N ≥ 2`, or `FAST_MODE`, or preferred issue / user constraints force supersession analysis
...
8. **Branch on mode after Phase 1 + 1.5 (+ S0 when required):**
   ...
   - Always run [Phase 1.5 — Scope](#phase-15--scope-milestone--group--area) when `REST` is non-empty (or to confirm `SCOPE` is unset).
...
2. Memory / recent commits / branch names with issue prefixes (`TW-123`)
3. Linear: `list_teams` → `list_projects` for that team; prefer the active engineering project, skip archived/completed umbrellas
...
## Phase 1.5 — Scope (milestone / group / area)

When `REST` is non-empty, resolve `SCOPE` using **Scope filter** in
[`references/eligibility.md`](references/eligibility.md) (`list_milestones`,
then labels, then area). Linear MCP: `search_tool` then `use_tool`.
...
`SOLVE_COUNT_MODE = all` and `GUIDANCE_REQUIRED = true`. Sequential default
is one in-scope leaf at a time until the drain gate (in-scope) passes. If
`FAST_MODE` and `--concurrency` was not passed, set `CONCURRENCY` to the
**`all fast` default (2)** after this implicit `all`.
...
Using Linear MCP (`list_issues`): follow **Linear inventory fetch** in
[`references/eligibility.md`](references/eligibility.md) (`list_issue_statuses`,
then `team` + `project` + `state` per actionable status, slim fields, page each
state). Do not list the project unfiltered. Fetch enough pages **per state** to
find the true lowest number. If `SCOPE` is set, keep only `issue_in_scope`
(same file, Scope filter) before 2B–2E.

### 2B. Eligible states

Canonical: [`references/eligibility.md`](references/eligibility.md) (Eligible states + On-dev check). Apply that filter; do not invent another. **In Review is not git** — run the on-dev check before skipping or treating the issue as a satisfied blocker.
...
Parse the identifier number: `TEAM-(\d+)` → integer. Sort **ascending** among
...
- Team/project used
- `SCOPE` if set (milestone/label/area) and that the rest of the board was ignored
...
4. **Required claim + start comment** on the leaf. `list_comments` first. Skip a new claim comment if this `run` already has a live `claimed-by:` on the issue. First line MUST be:
   `claimed-by: <bot-or-cli> · session <id> · worktree <cwd> · run <RUN_ID|sequential>`
   Then: why this order, epic expansion path if any, plan in 3–6 bullets, cheap implement + verify (deep review is `/prb`), delivery = **`origin/dev` then In Review (not Done)**, batch guidance path when S0 ran.
5. **Re-read the issue immediately.** If a different run's `claimed-by:` is newer, abort this leaf (do not code) and pick another.
6. **Required** brief comment on the parent epic when expanded (`list_comments` first; skip if a starting-child note for this leaf already exists): `Starting child TEAM-205 (lowest eligible) under this epic.` (skip if no parent).
7. When S0 **skips/cancels** an obsolete open ticket as full supersede: **one** comment on that ticket (`list_comments` first; skip if a superseded-by note already exists). Do not spam other skips.
...
1. **Required** completion comment on the leaf (`list_comments` first; skip if a completion note for this `origin/dev` SHA already exists):
   - Summary of code changes (paths)
   - Intensity band + inner-review none|bugs-only + runtime-proof evidence
...
- **Required** failure comment on the leaf (`list_comments` first) explaining what failed. In **integer `N`** multi-issue mode, note that the batch halted. In **`all`** mode, note that this leaf failed and the drain **continues** with other independent eligible issues.
...
- **`list_comments` immediately before every `save_comment`.** Skip when the same moment (claim `run`, completion SHA, rollup, superseded-by) is already on the thread.
...
- Confusing `--effort N` or `--concurrency N` (or auto-dialed inner-review / concurrency **2**) with solve count `N`
...
| `/solve` | Selects Linear issue(s) + **cheap construction** + merges to **local `dev`**; count via `/solve [N\|all]` (default 1); remainder can **scope** a milestone/label/area drain; optional **`fast`** parallel orchestrator. Deep review is `/prb`. |
...
| `/identify` | Human-approved 2–4 leaf batch; upgrades; JIT-claims; nested `/solve 1` (or `fast` when overlap-free) with `SELECTION_PIN` + shared `RUN_ID` |
